"""
app.py â€” AI Study Companion  (Streamlit front-end)
...
# â”€â”€â”€ Optional alert sound (winsound = stdlib on Windows) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
try:
    import winsound
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
with st.sidebar:
    st.markdown("### ðŸ¤– ML Data Collection")
...
# â”€â”€ LEFT: camera feed â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
with col_cam:
    ...
    cam_placeholder.markdown(
        '<div class="cam-placeholder">ðŸ“· Camera will appear here when session starts</div>',
        unsafe_allow_html=True,
    )
    ...
    with c1:
        start_btn = st.button("â–¶ Start Session", type="primary",
                              disabled=st.session_state["running"])
    with c2:
        stop_btn  = st.button("â¹ Stop Session",
                              disabled=not st.session_state["running"])
    with c3:
        save_btn  = st.button("ðŸ’¾ Save Summary",
                              disabled=st.session_state["session_stats"] is None)

# â”€â”€ RIGHT: dashboard â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
with col_dash:
    # Current state card
    st.markdown('<div class="glass-card">', unsafe_allow_html=True)
    ...
    pose_ph.markdown(
        '<div class="stat-row">'
        + render_stat_tile("EAR",   "â€”")
        + render_stat_tile("Pitch", "â€”")
        + render_stat_tile("Yaw",   "â€”")
        + render_stat_tile("Roll",  "â€”")
        + "</div>",
        unsafe_allow_html=True,
    )
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    if stats is None or stats.total_sec() < 5:
        ...
        if prompt := st.chat_input("Ask me for study tips or to analyze your session..."):
            ...
            if "summary" in p_lower or "analyze" in p_lower or "how did i do" in p_lower:
                if f_pct >= 80:
                    response = f"ðŸŒŸ **Excellent Session!** You were focused **{f_pct}%** of the time. You kept distractions to a minimum. Keep up this amazing momentum!"
                elif d_pct > 30:
                    response = f"ðŸ‘€ **Distraction Alert!** You were distracted for **{d_pct}%** of the session. Try putting your phone in another room or blocking distracting websites."
                elif s_pct > 10:
                    response = f"ðŸ˜´ **Drowsiness Detected.** You spent **{s_pct}%** of the session sleepy. It might be time to take a quick 20-minute power nap or grab some coffee!"
                else:
                    response = f"ðŸ“Š **Session Overview:** You were focused for **{f_pct}%**, distracted for **{d_pct}%**, and sleepy for **{s_pct}%**. A solid effort, but there is still room to improve your deep work streaks."
            elif "tip" in p_lower or "improve" in p_lower or "help" in p_lower:
                if d_pct > f_pct:
                    response = "ðŸ’¡ **Study Tip:** Since you get distracted easily, try the **Pomodoro Technique**: 25 minutes of intense focus followed by a 5-minute break. Don't touch your phone until the break!"
                elif s_pct > 0:
                    response = "ðŸ’¡ **Study Tip:** To fight drowsiness, ensure your study room is brightly lit, sit up straight, and stay hydrated. Avoid studying in bed at all costs."
                else:
                    response = "ðŸ’¡ **Study Tip:** You are doing great! To reach the next level of focus, try listening to binaural beats or lo-fi music without lyrics to block out background noise."
            else:
...
if save_btn and st.session_state["session_stats"]:
    ...
    path = save_session_summary(st.session_state["session_stats"],
                                f"session_{ts}.csv")
    st.success(f"âœ… Session saved to `{path}`")
...
if st.session_state["running"]:
    ...
    with vision.FaceLandmarker.create_from_options(options) as face_landmarker:
        ...
        while st.session_state["running"]:
            ...
            # â”€â”€ MediaPipe inference â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            if result.face_landmarks:
                ...
                # â”€â”€ ML Data Collection â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            # â”€â”€ Accumulate session stats â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            new_alerts = stats.update(state)

            # â”€â”€ Alert handling â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            for a in new_alerts:
            ...
            # â”€â”€ Annotate frame â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            pose_ph.markdown(
                ...
                + render_stat_tile("Pitch", f"{pitch:+.1f}Â°")
                + render_stat_tile("Yaw",   f"{yaw:+.1f}Â°")
                + render_stat_tile("Roll",  f"{roll:+.1f}Â°")
                + "</div>",
                unsafe_allow_html=True,
            )
            ...
            if events:
                ...
                for ts, st_ev in events:
                    ...
                    rows += (
                        ...
                        f'<span class="event-time">â± {t_str}</span>'
